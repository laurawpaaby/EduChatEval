...
def chat_with_model(
    api_url, model_name, temperature, context, participant_id, output_path
):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run interactive chat with a language model."
    )
    parser.add_argument(
        "--api_url", type=str, required=True, help="API endpoint for the language model"
    )
    parser.add_argument(
        "--model_name",
        type=str,
        required=True,
        help="Model name (e.g., llama-3.2-3b-instruct:2)",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.7,
        help="Temperature setting for model responses",
    )
    parser.add_argument(
        "--context", type=str, required=True, help="Context description for the model"
    )
    parser.add_argument(
        "--participant_id",
        type=str,
        required=True,
        help="Unique identifier for the participant",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        required=True,
        help="Path to save the conversation log",
    )
